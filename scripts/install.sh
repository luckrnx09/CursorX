...
install_macos() {
  ...
  curl -fsSL -o "${tmpdir}/CursorX.dmg" "$dmg_url"

  hdiutil attach "${tmpdir}/CursorX.dmg" -nobrowse -readonly -mountpoint "$mount_dir" -quiet

  if ! rm -rf "/Applications/${APP_NAME}.app" 2>/dev/null; then
    echo "Permission denied; retrying with sudo..."
    sudo rm -rf "/Applications/${APP_NAME}.app"
  fi
  if ! cp -R "${mount_dir}/CursorX.app" /Applications/ 2>/dev/null; then
    sudo cp -R "${mount_dir}/CursorX.app" /Applications/
  fi
...
install_linux() {
  ...
  local target="${HOME}/.local/bin/cursorx"

  echo "Downloading ${appimage_url}..."
  curl -fsSL -o "$target" "$appimage_url"
  chmod +x "$target"

  cat > "${HOME}/.local/share/applications/cursorx.desktop" <<EOF
[Desktop Entry]
Name=${APP_NAME}
Comment=A modern cross-platform desktop app for cursor highlighting
Exec=${target}
Type=Application
Categories=Utility;
Terminal=false
